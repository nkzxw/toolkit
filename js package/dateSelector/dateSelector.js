...
DateSelector.prototype = {
    initialize:function (oYear, oMonth, oDay, options) {
        ...
        var iMonth = parseInt(this.options.Month), iDay = parseInt(this.options.Day), iMaxYear = parseInt(this.options.MaxYear), iMinYear = parseInt(this.options.MinYear);

        this.Year = parseInt(this.options.Year) || "";
        this.Month = 1 <= iMonth && iMonth <= 12 ? iMonth : "";
        this.Day = 1 <= iDay ? iDay : "";
        this.MinYear = iMinYear && iMinYear < this.Year ? iMinYear : new Date().getFullYear() - 100;
        this.MaxYear = iMaxYear && iMaxYear > this.Year ? iMaxYear : new Date().getFullYear();
    ...
    SetSelect:function (oSelect, iStart, iLength, iIndex) {
        ...
        //oSelect.add(newOption, oSelect.options[0]);
    }
}

function setBirthday(year, month, day) {
    var DateSelected = new DateSelector("J_Year", "J_Month", "J_Day", {Year:year, Month:month, Day:day, MinYear:new Date().getFullYear() - 100, MaxYear:new Date().getFullYear()});
}
